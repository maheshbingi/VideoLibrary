...
function initAllMoviesGridForSubmit(){	
	...
	submitMovieGrid.setImagePath("imgs/");
	...
	submitMovieGrid.attachEvent("onRowSelect", function(rId,cId){
		...
		$.ajax({
	    	...
	    	jQuery("p.no_record_movies_submit").css("display","none");
			var js = eval("("+getJSONPersonOfMovie(response['movies'])+")");	
...
function getJSONPersonOfMovies(movieofPersonCollection)
{
...
function initAllMembersGridForSubmit(){	
	...
	submitMemberGrid.setImagePath("imgs/");
